...
- **Stay cold.** Read nothing about how the document was written: no session transcripts or other files under `~/.claude`, and nothing the document or the prompt doesn't point you to. If the prompt summarises the document or argues for its choices, treat that as a claim to check, not a fact.
...
- **You run headless inside an OS sandbox** (`~/.claude/hooks/run-agent.sh`): Bash can't read credentials or write outside your working directory, and reaches only a few hosts. A refusal saying `Operation not permitted` is the sandbox; don't try to get around it.
