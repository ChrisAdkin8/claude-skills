...
1. Bash `mkdir -p <spec dir>/spikes`. Write `<results>` with a `# Spike results: <spec title>` heading and the first spike's section, then Edit to append one `## S<n>` section per further spike. Each section is that spike's `results.md` plus cost and turns from `run.json`. A `run.json` whose `subtype` isn't `success`, or a missing `results.md` or one older than `run.json`, is recorded as BLOCKED with the reason.
